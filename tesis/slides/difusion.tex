\subsubsection{Difusión}
\begin{frame}
\frametitle{}

\begin{figure}
\begin{tikzpicture}[node distance=0.5cm, auto,>=latex', thick]
\scriptsize
    % We need to set at bounding box first. Otherwise the diagram
    % will change position for each frame.
    \path[use as bounding box] (-1.5,0) rectangle (12,-2);

    % TT methodology     
    \node [phase]                        (monitoreo)     {Vigilancia};
    \node [phase, below of=monitoreo]    (choice)        {Elección};
    \node [phase, below of=choice]       (acquisition)   {Adquisición};
    \node [phase, below of=acquisition]  (adaptation)    {Adaptación};
    \node [phase, below of=adaptation]   (absortion)     {Absorción};
    \node [phase, below of=absortion]    (aplication)    {Aplicación};
    \node [phase2,below of=aplication]   (difusion)      {Difusión};

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&
    %            Difusión
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&


    \onslide<1> \node [difusion_es, right=1cm of absortion.east] (est_difusion)      
      {
       \begin{center} \textbf{El conocimiento como bien público} \end{center}
       \includegraphics[scale=.4]{../images/bienes_clasific.png}\\
      };


    \onslide<2> \node [difusion_es, right=.5cm of absortion.east] (est_difusion)      
      {
       \begin{center} \textbf{Marco para analizar el conocimiento como bien público aplicado a la metodología propuesta} \end{center}
       \begin{center} \includegraphics[scale=.47]{../images/framework_hwcl.pdf} \end{center}
       \begin{center} Ecuación costo-beneficio de Ostrom: \textbf{BC > BN + C\footnote{BC: Beneficio al Contribuir, BN: Costo de no contribuir, C: Costo de la contribución}}\end{center}
      };



%     \onslide<2> \node [difusion_es, right=1cm of absortion.east] (est_difusion)      
%       {
%        \begin{center} \textbf{Hardware Copyleft vs. Software Libre} \end{center}
%           \setlength\minrowclearance{1pt}
%           \setlength\arrayrulewidth{1pt}
%           \begin{tabular}{|l|l|l|}
%             \hline
%             \rowcolor[rgb]{0.9,0.9,0.4}
%             \bf Concepto          &\bf Software           & \bf Hardware \\
%             \hline
%             Código Fuente         & Programa              & Esquemático y Layout \\
%             \rowcolor[rgb]{0.8,1,1}
%             Editor                & Editor de Texto       & Sistema EDA \\
%             Conversión            & Compilador, etc.      & Sistema EDA \\
%             \rowcolor[rgb]{0.8,1,1}
%             Prueba                & Ejecución             & Prototipo(s) \\
%             Depuración            & Debugger              & Laboratorio \\
%             \rowcolor[rgb]{0.8,1,1}
%             Reproducción          & Descarga              & Producción, \\
%             \rowcolor[rgb]{0.8,1,1}
%                                   & (Copia perfecta)      & Pruebas \\
%             Distribución          & Internet              & Envíos, Aduanas \\
%             \hline
%           \end{tabular}
% 
%       };

    \onslide<3> \node [ph_explain2, right=.5cm of absortion.east] (est_difusion)      
      {
       \begin{center} \textbf{Hardware Copyleft: Las cuatro libertades $[1]$} \end{center}

          \begin{enumerate}
            \item[0] Ejecutar el programa
              \begin{itemize}
                \scriptsize
                \item Usar el hardware
              \end{itemize}
            \item[1] Estudiar el código
              \begin{itemize}
                \scriptsize
                \item Estudiar los archivos de diseño (Esquemático y layout)
              \end{itemize}
            \item[1] Adaptar el código fuente
              \begin{itemize}
                \scriptsize
                \item Adaptar los archivos de diseño
                \item Tener acceso a las herramientas para hacerlo
              \end{itemize}
            \item[2,3] Redistribuir copias/modificaciones
              \begin{itemize}
                \scriptsize
                \item Redistribuir los archivos de diseño
                \item Construir o producir el hardware
              \end{itemize}
          \end{enumerate}

          {\scriptsize $[1]$~\url{http://www.gnu.org/philosophy/free-sw.html}}

      };

%     \onslide<4> \node [ph_explain2, right=.5cm of absortion.east] (est_difusion)      
%       {
%        \begin{center} \textbf{Hardware Copyleft: Requerimientos} \end{center}
%        \includegraphics[scale=.5]{../images/copy_left_hardware_requirement.pdf}\\
% %        {\scriptsize Fuente: Werner Almesberger  FISL 12, Brazil}
%       };

    \onslide<4> \node [ph_explain2, right=.1cm of absortion.east] (est_difusion)      
      {
       \begin{center} \textbf{Flujo de diseño utilizando hardware copyleft} \end{center}
       \begin{center} \includegraphics[scale=.48]{../images/design_flow_copyleft_hw.pdf} \end{center}
      };




    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&
    %        Estrategias de difusión
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&
    \onslide<5>
      \node [difusion_es, above right=1cm of adaptation.east] (est_difusion)      
      {
       \begin{center} \textbf{Linuxencaja: Estrategia de Difusión} \end{center}
       \includegraphics[scale=.25]{../images/plataformas_hwcl.png}\\
       \color{blue!40!black} {Repositorios + Tutoriales + Cursos en línea}\\
       \color{green!60!black}{Código fuente (.c,.v,.vhd,.S,.java), archivos de diseño} 

      };

     \node [difusion_es, below = 0.25cm of est_difusion] (canales_difusion)      
      {
       \color{red!60!black} {Canales de difusión} \\
                          GIT, WIKI, MAIL
      };

     \draw[myarrow] (est_difusion.south) -- (canales_difusion.north);   



     \node [difusion_es, below = 0.25cm of canales_difusion] (miembros)      
      {
       \begin{center} \includegraphics[scale=.4]{../images/community_members.pdf} \end{center}

      };

      \draw[myarrow] (canales_difusion.south) -- (miembros.north);   

    
%      \node [phase2, below = .6cm of canales_difusion] (dif_academia2)      
%       {
%         Academia \\
%         \begin{itemize}
%          \scriptsize
%          \item UIS, ULA, USTA, UDFJC, ENAP.
%          \item ET-ITC.
%          \item SEB.
%         \end{itemize}
%       };
%      \draw[myarrow] (canales_difusion.south) -- (dif_academia2.north);	
%      \node [phase2, left = .1cm of dif_academia2] (dif_academia)      
%       {
%         I+D UNAL \\
%         \begin{itemize}
%          \scriptsize
%          \item Medicina
%          \item Veterinaria
%          \item Mecatrónica
%          \item Sistemas
%         \end{itemize}
%       };
%      \draw[myarrow] (canales_difusion.south) --++(0,-0.2)--++ (-2.3cm, 0) -- (dif_academia.north);	
%      \node [phase2, right = .1cm of dif_academia2] (dif_industria)      
%       {
%         Industria\\
%          \scriptsize
%         \begin{itemize}
%          \item emQbit.
%          \item $\mu$Ensamble.
%         \end{itemize}
%       };
%      \draw[myarrow] (canales_difusion.south) --++(0,-0.2)--++ (2.3cm, 0) -- (dif_industria.north);

 
  




\end{tikzpicture}
\end{figure}

\end{frame}